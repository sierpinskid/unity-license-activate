...
const cli_md = function () {
    ...
    let emailHost = args.getArg("--host", null);

    console.log("[INFO] email: " + email);
    console.log("[INFO] emailHost: " + emailHost);
    console.log("[INFO] alf: " + alf);
    ...
    activate.start(email, password, alf, verification, emailPassword, emailHost)
        .catch((error) => {
            console.log("[ERROR] start failed: " + error);
        });
};
