...
async function getVerification(email, password, host, count = 0) {
    ...
    try {
        ...
        execSync(`unity-verify-code "${email}" "${password}" "${savePath}" ${hostArg}`, {stdio: 'inherit'});
...
async function tryAcceptTos(page) {

    console.log('[INFO] Check if TOS accept is needed..');

    const needTosAccept = await page.$('#accept-updated-tos');
    ...
    if (needTosAccept) {
        ...
        await Promise.all([
            ...
            page.$eval('#new_conversations_accept_updated_tos_form .accept', btn => btn.click())
        ]);
    }
}
...
async function start(email, password, alf, verification, emailPassword, emailHost, count = 0) {
    ...
    try {
        ...
        if (needVerify) {
            console.log('[INFO] Detected verification code needed, retrieve it from Email...');
        ...
        if (count < maxAttempts) {
            ...
            try {
                ...
                console.error('Recursive start failed:', recursiveError);
            }
            return;
        }
